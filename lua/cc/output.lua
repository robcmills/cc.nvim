...
do
  local ft = require('cc.output.foldtext')
  M.foldexpr = ft.foldexpr
  M.foldtext = ft.foldtext
  M.default_foldtext = ft.default_foldtext
  M.refresh_carets = ft.refresh_carets
end
...
function Output:_render_tool_input(tool_name, input)
  ...
  if not body_lines then
    local body = require('cc.output.tool_body').default_tool_body(tool_name, input)
...
function Output:render_result(result)
  ...
  if text == nil then text = require('cc.output.cost').default_format(result) end
