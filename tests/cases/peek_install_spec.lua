...
T['install']['writes settings.json via atomic rename (no .tmp left behind)'] = function()
  local p = setup_temp_home(_G.child)
  _G.child.lua([[require('cc.peek').install()]])

  -- After install, no stale .cc-peek.tmp.* file should remain in the
  -- settings dir. Atomic write writes to a tmp file then renames it into
  -- place; failure to do so would leave the tmp file behind.
  local settings_dir = _G.child.lua_get(string.format(
    [[vim.fn.fnamemodify(%q, ':h')]], p.settings))
  local stragglers = _G.child.lua_get(string.format(
    [[vim.fn.glob(%q, false, true)]],
    settings_dir .. '/settings.json.cc-peek.tmp.*'))
  eq(#stragglers, 0)
end
